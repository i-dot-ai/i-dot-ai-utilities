...
class Settings(BaseSettings):
    ...
    - **IAI_MINIO_ADDRESS**: The minio host address (for localhost, only needs setting if not using the default `localhost` minio address)
    - **IAI_S3_AWS_ACCESS_KEY_ID**: The AWS access key ID (for localhost, only needs setting if not using the default minio credentials)
    - **IAI_S3_AWS_SECRET_ACCESS_KEY**: The AWS secret key (for localhost, only needs setting if not using the default minio credentials)
    - **IAI_DATA_DIR**: The data directory to use inside the set S3 bucket (defaults to `app_data`)
    ...
    def boto3_client(self) -> boto3.client:
        """
        This function returns the client connection to S3 or minio using boto3, depending on the environment variable `ENVIRONMENT`
